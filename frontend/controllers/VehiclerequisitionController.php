...
class VehiclerequisitionController extends Controller
{
        ...
        return [
            'access' => [
                ...
                'rules' => [
                    ...
                    [
                        'actions' => ['logout','index','advance-list','create','update','delete','view','approved-requisitions'],
                        'allow' => true,
                        'roles' => ['@'],
                    ],
                ],
   ...
   // Get Vehicle Requisition list
    ...
    public function actionAvailabilityList(){
        $service = Yii::$app->params['ServiceName']['Vehicle_Availability_Status'];
        $filter = [];
    ...
    public function actionApprovedList(){
        ...
        foreach($results as $item){
            ...
            $releaselink = Html::a('<i class="fas fa-check"></i>',['release-booking','No'=> $item->Booking_Requisition_No ],['class'=>'btn btn-outline-primary btn-xs',
                'data' => [
                    'confirm' => 'Are you sure you want to release this vehicle booking ?'
                ]
                ,'title' => 'Release this Vehicle Booking.' ]);
            ...
            if(!empty($item->Booking_Requisition_No ))
            {
                $result['data'][] = [
                    ...
                    'Requisition_Date' => !empty($item->Requisition_Date)?$item->Requisition_Date:'',
                    'Vehicle_Registration_No' => !empty($item->Vehicle_Registration_No)?$item->Vehicle_Registration_No:' Not Set',
                    'Reason_For_Booking' => !empty($item->Reason_For_Booking)?$item->Reason_For_Booking:'',
    ...
    public function actionReleaseBooking($No)
    {
        ...
        $result = Yii::$app->navhelper->Codeunit($service,$data,'ReleaseVehicleBookings');

        if(!is_string($result)){
            Yii::$app->session->setFlash('success', 'Vehicle Booking Released Successfully.', true);
            return $this->redirect(['approved-requisitions']);
        }else{

            Yii::$app->session->setFlash('error', 'Error Releasing Vehicle Booking  : '. $result);
            return $this->redirect(['approved-requisitions']);
