...
$script = <<<JS
    ...
    // make reliever list searchable
    
    $('#leave-reliever').select2();
    ...
    // Set Leave Type
     ...
     $('#leave-include_leave_allowance').change(function(e){
        const Include_Leave_Allowance = e.target.value;
        const No = $('#leave-application_no').val();
        if(No.length){
            
            const url = $('input[name=url]').val()+'leave/setleaveallowance';
            $.post(url,{'Include_Leave_Allowance': Include_Leave_Allowance,'No': No}).done(function(msg){
                   //populate empty form fields with new data
                   ...
                   $('#leave-key').val(msg.Key);
                    ...
                    console.table(msg);
                    if((typeof msg) === 'string') { // A string is an error
                        const parent = document.querySelector('.field-leave-include_leave_allowance');
                        const helpbBlock = parent.children[2];
                        helpbBlock.innerText = msg;
                        disableSubmit();
